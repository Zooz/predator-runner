...
describe('integration tests', () => {
    ...
    describe('Successfully run a test with csv file', function () {
        ...
        const basicTestPath = path.resolve(__dirname, '../../test-scripts/basic_test_with_csv.json');
        const csvPath = path.resolve(__dirname, '../../test-scripts/id_to_name.csv');
        const csv = fs.readFileSync(csvPath, 'ascii');
        const testBody = require(basicTestPath);
        testBody.csv_file_id = csvFileId;
        nock(PREDATOR_URL)
            .get(`/tests/${testId}`)
            .times(1)
            .reply(200, testBody);
        ...
        const uriList = [];
        nock('http://www.predator.dev')
            .get((uri) => uriList.push(uri))
            .reply(200, {})
            .persist();
        ...
        it('Test should use the correct values from csv', async function () {
